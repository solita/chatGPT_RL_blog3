...
from keras.layers import (Conv1D, Dense, Dropout, Flatten, Input, Lambda,
	...
	def _build_model(self):
		...
		if num_devices > 2:
			...
			with strategy.scope():
				...
				x = Reshape((self.state_size, 1))(input_state)
				x = Conv1D(32, kernel_size=3, activation='swish',
						   input_shape=(self.state_size, 1))(x)
				x = MaxPooling1D(pool_size=2)(x)
				x = Conv1D(64, kernel_size=3, activation='swish')(x)
				x = MaxPooling1D(pool_size=2)(x)
				x = Flatten()(x)
				x = Dense(64, activation='swish', kernel_initializer="glorot_uniform",
